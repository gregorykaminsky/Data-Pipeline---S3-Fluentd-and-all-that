...
def allowed_keys(key):
    for camp in email_campaigns:
        if key.find(camp) >= 0: return True
    return False
...
def log_errors(error_keys, client, bucket, link2):
    if error_keys != {}:
        ...
        error_file = open("/tmp/" + name_error_file, 'w')
        error_file.write(json.dumps(error_keys))
        error_file.close()
...
def lambda_handler(event, context):
    ...
    for record in event['Records']:
        ...
        table = getTable(key) ''' Get the table where this file is suppose to go '''
    ...
    for table in all_keys:
        json_output = recursive_read_aws.read_then_to_json(client = client, file_names = all_keys[table], bucket = bucket, error_keys = error_keys)
